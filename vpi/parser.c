...
//    printf("progcntr: %0d\n", progcntr);
    if (lparser->currentToken.kind == ID) {
      ...
      else if (lparser->currentToken.kind == NOOP ||
               lparser->currentToken.kind == DOUT ||
               lparser->currentToken.kind == DIN ||
               lparser->currentToken.kind == AOUT ||
               lparser->currentToken.kind == AIN ||
               lparser->currentToken.kind == HOUT ||
               lparser->currentToken.kind == HIN ||
               lparser->currentToken.kind == SOUT ||
...
void program(parserData *lparser) {
  ...
  else if (lparser->currentToken.kind == BPBP) {
    bpbp(lparser);
    if (lparser->errorcode == -1) {
      return;
    }
  ...
  else if (lparser->currentToken.kind == POBP) {
    pobp(lparser);
    if (lparser->errorcode == -1) {
      return;
    }
  ...
  else if (lparser->currentToken.kind == PBP) {
    pbp(lparser);
    if (lparser->errorcode == -1) {
      return;
    }
  ...
  else if (lparser->currentToken.kind == HIN) {
    hin(lparser);
    if (lparser->errorcode == -1) {
      return;
    }
  ...
  else if (lparser->currentToken.kind == HOUT) {
    hout(lparser);
    if (lparser->errorcode == -1) {
      return;
    }
  ...
  else if (lparser->currentToken.kind == AIN) {
    ain(lparser);
    if (lparser->errorcode == -1) {
      return;
    }
  ...
  else if (lparser->currentToken.kind == AOUT) {
    aout(lparser);
    if (lparser->errorcode == -1) {
      return;
    }
  ...
  else if (lparser->currentToken.kind == DIN) {
    din(lparser);
    if (lparser->errorcode == -1) {
      return;
    }
  ...
  else if (lparser->currentToken.kind == DOUT) {
    dout(lparser);
    if (lparser->errorcode == -1) {
      return;
    }
...
void dout(parserData *lparser) {
  assert(consume(lparser, DOUT) == 0);
  if (lparser->cg->symD == NULL) {
    lparser->cg->symD = symDataNew();
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
    codeGenEmmitInstruction(lparser->cg, cgTypeDOUT, "dout");
    symDataDelete(&(lparser->cg->symD));
  }
  else {
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
...
void din(parserData *lparser) {
  assert(consume(lparser, DIN) == 0);
  if (lparser->cg->symD == NULL) {
    lparser->cg->symD = symDataNew();
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
    codeGenEmmitInstruction(lparser->cg, cgTypeDIN, "din");
    symDataDelete(&(lparser->cg->symD));
  }
  else {
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
...
void aout(parserData *lparser) {
  assert(consume(lparser, AOUT) == 0);
  if (lparser->cg->symD == NULL) {
    lparser->cg->symD = symDataNew();
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
    codeGenEmmitInstruction(lparser->cg, cgTypeAOUT, "aout");
    symDataDelete(&(lparser->cg->symD));
  }
  else {
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
...
void ain(parserData *lparser) {
  assert(consume(lparser, AIN) == 0);
  if (lparser->cg->symD == NULL) {
    lparser->cg->symD = symDataNew();
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
    codeGenEmmitInstruction(lparser->cg, cgTypeAIN, "ain");
    symDataDelete(&(lparser->cg->symD));
  }
  else {
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
...
void hout(parserData *lparser) {
  assert(consume(lparser, HOUT) == 0);
  if (lparser->cg->symD == NULL) {
    lparser->cg->symD = symDataNew();
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
    codeGenEmmitInstruction(lparser->cg, cgTypeHOUT, "hout");
    symDataDelete(&(lparser->cg->symD));
  }
  else {
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
...
void hin(parserData *lparser) {
  assert(consume(lparser, HIN) == 0);
  if (lparser->cg->symD == NULL) {
    lparser->cg->symD = symDataNew();
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
    codeGenEmmitInstruction(lparser->cg, cgTypeHIN, "hin");
    symDataDelete(&(lparser->cg->symD));
  }
  else {
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
...
void sinn(parserData *lparser) {
  assert(consume(lparser, SIN) == 0);
  if (lparser->cg->symD == NULL) {
    lparser->cg->symD = symDataNew();
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
    codeGenEmmitInstruction(lparser->cg, cgTypeSIN, "sin");
    symDataDelete(&(lparser->cg->symD));
  }
  else {
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
...
void pobp(parserData *lparser) {
  assert(consume(lparser, POBP) == 0);
  if (lparser->cg->symD == NULL) {
    lparser->cg->symD = symDataNew();
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
    codeGenEmmitInstruction(lparser->cg, cgTypePOBP, "pobp");
    symDataDelete(&(lparser->cg->symD));
  }
  else {
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
...
void bpbp(parserData *lparser) {
  assert(consume(lparser, BPBP) == 0);
  if (lparser->cg->symD == NULL) {
    lparser->cg->symD = symDataNew();
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
    codeGenEmmitInstruction(lparser->cg, cgTypeBPBP, "bpbp");
    symDataDelete(&(lparser->cg->symD));
  }
  else {
    sprintf(lparser->cg->symD->programcounter, "%04x", lparser->addrCntr);
